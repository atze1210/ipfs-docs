...
::: tip Generated on 2026-06-08 18:19:39, from kubo 0.42.0
...
SYNOPSIS
  ipfs dag export [--progress | -p] [--local-only] [--] <root>
...
OPTIONS
  ...
  -p, --progress  bool - Stream progress data. Defaults to true when stderr is
                         a terminal.
  --local-only    bool - Best-effort export of locally-available blocks;
                         missing or unreadable blocks (and their subtrees) are
                         skipped. Implies --offline.
...
SYNOPSIS
  ipfs dag import [--pin-roots] [--local-only] [--silent] [--stats]
                  [--fast-provide-root] [--fast-provide-dag]
...
OPTIONS
  ...
  --local-only         bool - Import a partial CAR (e.g. from 'dag export
                              --local-only'). Implies --pin-roots=false.
  ...
  --fast-provide-root  bool - Immediately provide root CIDs to DHT in addition
                              ...
                              Import.FastProvideRoot.
  --fast-provide-dag   bool - Walk and provide the full DAG according to
                              Provide.Strategy after import. Default:
...
SUBCOMMANDS
  ...
  ipfs diag healthy - Report whether the daemon is operational.
...
EXPERIMENTAL SUBCOMMANDS
  ...
  ipfs diag datastore put <key> <value> - Write a raw key-value pair to the
                                          datastore.
...
SYNOPSIS
  ipfs diag datastore put [--] <key> <value>

ARGUMENTS

  <key>   - Datastore key (e.g., /test/mykey)
...
```
USAGE
  ipfs files chcid <path> - Change the CID version or hash function of the root
...
ARGUMENTS
  ...
  <path> - Path to change (must not be '/').
...
SYNOPSIS
  ipfs get [--output=<output> | -o] [--archive | -a] [--compress | -C]
           [--compression-level=<compression-level> | -l] [--progress | -p] [--]
           <ipfs-path>
...
OPTIONS
  ...
  -p, --progress           bool   - Stream progress data. Defaults to true when
...
SYNOPSIS
  ipfs object patch add-link [--create | -p] [--allow-non-unixfs | -] [--]
                             <root> <name> <ref>
...
OPTIONS
  ...
  --, --allow-non-unixfs  bool - Skip UnixFS validation, allowing links on
                                 non-directory nodes.
...
SYNOPSIS
  ipfs object patch rm-link [--allow-non-unixfs | -] [--] <root> <name>
...
OPTIONS
  ...
  --, --allow-non-unixfs  bool - Skip UnixFS validation, allowing link removal
                                 on non-directory nodes.
...
SYNOPSIS
  ipfs pin add [--recursive=false] [--name=<name> | -n] [--progress]
               [--fast-provide-root] [--fast-provide-dag] [--fast-provide-wait]
               [--] <ipfs-path>...
...
OPTIONS
  ...
  --fast-provide-root  bool   - Immediately provide root CID to DHT after
                                pinning. Default: Import.FastProvideRoot.
  --fast-provide-dag   bool   - Walk and provide the full DAG according to
                                Provide.Strategy after pinning. Default:
                                Import.FastProvideDAG.
  --fast-provide-wait  bool   - Block until the immediate provide completes.
...
SYNOPSIS
  ipfs pin update [--unpin=false] [--fast-provide-root] [--fast-provide-dag]
                  [--fast-provide-wait] [--] <from-path> <to-path>
...
OPTIONS
  ...
  --fast-provide-root  bool - Immediately provide new root CID to DHT after
                              update. Default: Import.FastProvideRoot.
  --fast-provide-dag   bool - Walk and provide the full DAG according to
                              Provide.Strategy after update. Default:
                              Import.FastProvideDAG.
  --fast-provide-wait  bool - Block until the immediate provide completes.
                              Default: Import.FastProvideWait.
...
EXPERIMENTAL SUBCOMMANDS
  ...
  ipfs provide once <cid>... - Announce CIDs to the routing system on demand.
  ipfs provide stat          - Show statistics about the provide system
...
SYNOPSIS
  ipfs provide once [--recursive | -r] [--] <cid>...

ARGUMENTS

  <cid>... - The CID(s) to announce.

OPTIONS

  -r, --recursive  bool - Recursively announce the entire DAG.
...
DEPRECATED SUBCOMMANDS
  ipfs routing provide <key>... - Deprecated, use 'ipfs provide once' instead.
  ipfs routing reprovide        - Trigger a reprovide cycle (legacy provider
                                  only).
...
USAGE
  ipfs routing reprovide - Trigger a reprovide cycle (legacy provider only).
...
USAGE
  ipfs update - Update Kubo to a different version
...
EXPERIMENTAL SUBCOMMANDS
  ipfs update check               - Check if a newer Kubo version is available
  ipfs update clean               - Remove backups of previous Kubo versions
  ipfs update install [<version>] - Download and install a Kubo update
  ipfs update revert              - Revert to a previously installed Kubo
                                    version
  ipfs update versions            - List available Kubo versions
...
SYNOPSIS
  ipfs update install [--pre] [--allow-downgrade] [--] [<version>]
...
OPTIONS
  ...
  --pre              bool - Include pre-release versions when resolving latest.
  --allow-downgrade  bool - Allow installing an older version.
...
USAGE
  ipfs update revert - Revert to a previously installed Kubo version
...
SYNOPSIS
  ipfs update versions [--count=<count> | -n] [--pre]

OPTIONS

  -n, --count  int  - Number of versions to list. Default: 30.
  --pre        bool - Include pre-release versions.
